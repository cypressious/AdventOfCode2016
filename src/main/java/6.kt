...
fun solve(input: String, part1: Boolean) {
    ...
    val frequencies = Array(lines.first().length) { LongArray(26) }
    ...
    println(frequencies.joinToString("") {
        val offset = if (part1) it.indexOfMax() else it.indexOfMin()
        ('a' + offset).toString()
    })
}

private fun LongArray.indexOfMax() = withIndex()
        .maxBy { it.value }!!
        .index

private fun LongArray.indexOfMin() = withIndex()
        .filter { it.value != 0L }
        .minBy { it.value }!!
        .index
